...
public class Mob {
    ...
    public void damageTrue(Player player, PlayerData playerData, int damage, SKRPG skrpg) {
        ...
        if (skrpg.getScrollBossManager().getBoss(this) != null) {
            skrpg.getScrollBossManager().getBoss(this).damage(damage, player);
        }
        ...
        damageIndictator.setCustomName(Text.color("&2" + damage + " &2&l☄ TRUE"));
    ...
    public void damage(Player player, int damage, SKRPG skrpg) {
        ...
        if (itemInfo != null) {
            ...
            if (itemInfo.getItem() == Items.SPIDER_BOW && !isPoisoned) {
                isPoisoned = true;
                new BukkitRunnable() {
                    int halfSeconds = 0;
                    @Override
                    public void run() {
                        if (getEnt().isDead()) { cancel(); return; }
                        if (halfSeconds == 40) {
                            isPoisoned = false;
                            cancel();
                        }
                        damageTrue(player, playerData, 20, skrpg);
                        if (getEnt() instanceof Creature) {
                            ((Creature) getEnt()).damage(0.1f);
                            ((Creature) getEnt()).setHealth(((Creature) getEnt()).getMaxHealth());
                        }
                        player.playSound(getEnt().getLocation(), Sound.ENTITY_SPIDER_HURT, 1.0f, 0.2f);
                        halfSeconds++;
                    }
