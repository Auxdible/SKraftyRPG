...
public class LordBryan implements NPC {
    ...
    public LordBryan(SKRPG skrpg, Location location) {
        this.skrpg = skrpg;
    ...
    public void onInteract(Player p, PlayerData playerData, SKRPG skrpg) {
        ...
        if (playerData.hasQuest(Quests.VALISSA_ARACHNE_QUEST) && playerData.getQuest(Quests.VALISSA_ARACHNE_QUEST).getPhase() == 3) {
            playerData.getQuest(Quests.VALISSA_ARACHNE_QUEST).setPhase(4);
                Quests.VALISSA_ARACHNE_QUEST.getQuest().executePhase(p, skrpg);
        } else if (!playerData.hasQuest(Quests.LORD_BRYAN) && !playerData.getCompletedQuests().contains(Quests.LORD_BRYAN)) {
            Quests.startQuest(Quests.LORD_BRYAN, p, playerData, skrpg);
        } else if (playerData.hasQuest(Quests.LORD_BRYAN)) {
