...
public class Totem3 implements NPC {
    private Entity entity;
    private SKRPG skrpg;
    private Location location;
    private Entity hitbox, clickStand;
    public Totem3(SKRPG skrpg, Location location) {
        this.skrpg = skrpg;
    ...
    public void buildNPC(SKRPG skrpg) {
        ...
        nameStand.setCustomName(Text.color("&aTotem 3"));
    ...
    public void onInteract(Player p, PlayerData playerData, SKRPG skrpg) {
        if (playerData.hasQuest(Quests.TUTORIAL)) {
            ...
            if (!tutorialQuest.getTotemsInteracted().contains(3)) {
                Text.applyTypingTitle(skrpg, p, Text.color("&7/ct"), 5, 20);
                Text.applyTextWithCommand(p, "&a&lCLICK ME! &r&7/ct", "/ct");
                tutorialQuest.addTotem(3);
            }
        }
