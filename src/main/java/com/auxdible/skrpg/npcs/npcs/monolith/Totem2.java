...
public class Totem2 implements NPC {
    private Entity entity;
    private SKRPG skrpg;
    private Location location;
    private Entity hitbox, clickStand;
    public Totem2(SKRPG skrpg, Location location) {
        this.skrpg = skrpg;
    ...
    public void buildNPC(SKRPG skrpg) {
        ...
        nameStand.setCustomName(Text.color("&aTotem 2"));
    ...
    public void onInteract(Player p, PlayerData playerData, SKRPG skrpg) {
        if (playerData.hasQuest(Quests.TUTORIAL)) {
            ...
            if (!tutorialQuest.getTotemsInteracted().contains(2)) {
                Text.applyTypingTitle(skrpg, p, Text.color("&7/trade (player, accept, deny)"), 5, 20);
                Text.applyTextWithCommand(p, "&a&lCLICK ME! &r&7/trade (player, accept, deny)", "/trade");
                tutorialQuest.addTotem(2);
            }
        }
