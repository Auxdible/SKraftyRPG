...
public class NPCManager {
    ...
    public void enable() throws InstantiationException, IllegalAccessException {
        ...
        for (String s : skrpg.getConfig().getConfigurationSection("npcs").getKeys(false)) {
            ...
            addNpc(npcType, x, y, z, yaw, pitch);
        }
    }
    public void disable() {
        for (NPC npc : npcs) {
    ...
    public void addNpc(String type, double x, double y, double z, float yaw, float pitch) throws IllegalAccessException, InstantiationException {
        ...
        try {
            ...
            npc = (NPC) npcClass.getDeclaredConstructor(cArg).newInstance(skrpg, new Location(Bukkit.getWorld(skrpg.getConfig().getString("rpgWorld")), x, y, z, yaw, pitch));
