...
public class Fishing {
    ...
    public Fishing(Level level, double totalXP, double xpTillNext) {
        this.level = level;
    ...
    public void levelUpSkill(Player player, PlayerData playerData, SKRPG skrpg) {
        if (getLevel() != Level._50 && getXpTillNext() >= Level.valueOf("_" +
            ...
            Text.applyText(player, "&7You leveled up to Fishing &e" + getLevel().toString().replace("_", ""));
            Text.applyText(player, "&7Earned &f+1 Speed ≈ &r&7and &60.5% more treasure chance!");
            Text.applyText(player, "&7+ &6" + creditsEarned + " Nuggets");
            Text.applyText(player, "&7Total Treasure Chance: &6" + (0.5 *
                    Integer.parseInt(getLevel().toString()
                            .replace("_", ""))) + "%");
            Text.applyText(player, "&8&m>                                          ");
            playerData.setBaseSpeed(playerData.getBaseSpeed() + 1);
